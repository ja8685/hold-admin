...
sql=" select count(a.number) from PointChangeLog a with(nolock) inner join v_member_list01 b with(nolock) on a.user_id=b.user_id where a.date between '"&sdate&"' and '"&ldate&"'"
if keyword <> "" then 
                ...
                <div class="x_panel">
                  ...
                  <div class="x_content">                        
                   ...
                   <script type="text/javascript">
                      ...
                      <%else 
                         sql=" select top "&PageSize&" a.*,b.jisa_id,b.chongpan_id,b.shop_id from PointChangeLog a with(nolock) inner join v_member_list01 b with(nolock) on a.user_id=b.user_id where a.date between '"&sdate&"' and '"&ldate&"' "&searchSQL&" and a.number not in (select top "&int((page-1)*PageSize)&" a.number from PointChangeLog a with(nolock) inner join v_member_admin_list b with(nolock) on a.user_id=b.user_id where  a.date between '"&sdate&"' and '"&ldate&"' "&searchSQL&" order by a.date desc) order by date desc"
