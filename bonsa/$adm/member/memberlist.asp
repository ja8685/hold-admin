...
If realuser = "Y" Then
	...
	searchSQL1 = searchSQL1 & " and user_distributor15_link <> '228' "
End If
...
sql=sql&" user_black_list,a.user_state,a.number,a.user_degree , ROW_NUMBER() OVER (ORDER BY "&orderby&" DESC) AS RowNum, "
sql=sql&" (SELECT count(number) FROM users a"
sql = sql & " where 1=1"
if keyword <> "" then 
	sql = sql & " and " & keyfield & " like '%"& keyword &"%'"
end if
sql=sql&searchSQL1
sql=sql&" ) as tot_cnt"
sql=sql&" ,user_distributor1_link,user_distributor15_link from users a with(nolock)"
...
sql=sql&searchSQL1&" ) a"
sql=sql&" where user_distributor1_link="&dis_no & searchSQL1
...
<!--
    ...
    function orderby(str) {
        ...
        if (str == 'R') {
            document.location.href = "<%=move_url%>&orderby=a.user_regdate"
        }
        if (str == 'S') {
                  ...
                  <div class="x_content">         
                      <form name="frm1" method="post" action="?" class="form-horizontal">
                            <div class="row">
                                <div class="form-inline">
                                    ...
                                    <div class="col-sm-2 pull-right">
                                        <label><input type="radio" name="realuser" class="radio" id="optionsRadios1"  value="N" <% If realuser = "N" Then Response.write "checked" End If %>>전체</label>
                                        <label><input type="radio" name="realuser" class="radio" id="optionsRadios2" value="Y" <% If realuser = "Y" Then Response.write "checked" End If %>/>실유저</label>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="form-inline form-group pull-right">
                                <button onclick="orderby('M')" class="btn btn-<%if orderby="" or orderby<>"user_money" then response.write "primary" else response.write "info" end if %> form-control">머니순정렬</button>
                                <button onclick="orderby('R')" class="btn btn-<%if orderby="" or orderby<>"a.user_regdate" then response.write "primary" else response.write "info" end if %> form-control">가입순정렬</button>
                                <button onclick="orderby('S')" class="btn btn-<%if orderby="" or orderby<>"user_save" then response.write "primary" else response.write "info" end if %> form-control">금고머니순정렬</button>
                                <button onclick="orderby('A')" class="btn btn-<%if orderby="" or orderby<>"shop_id" then response.write "primary" else response.write "info" end if %> form-control">매장ID순정렬</button>
                                &nbsp;&nbsp;&nbsp;&nbsp;
                            </div>
