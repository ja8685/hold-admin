<%
    ...
    function now_jungsandate(db_field)
    ltime = hour(now())
    if ltime<13 then
        strDateNow = "convert(varchar(10),"&db_field&",112)+Right('00' + RTrim(Cast(datepart(hour,"&db_field&") as char(2))),2)"
        strDateNow = strDateNow&" > convert(varchar(10),dateadd(day,-1,getdate()),112)+'12'"
    else
        strDateNow = "convert(varchar(10),"&db_field&",112)+Right('00' + RTrim(Cast(datepart(hour,"&db_field&") as char(2))),2)"
        strDateNow = strDateNow&" > convert(varchar(10),getdate(),112)+'12'"
    end if
        now_jungsandate = strDateNow
    end function 
