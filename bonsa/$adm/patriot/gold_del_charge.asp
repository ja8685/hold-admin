...
sql=sql&" select a.number from GiftToLog a with(nolock) inner join v_all_member_list b with(nolock) on a.from_user=b.user_id"
sql=sql&" where 1=1"
sql=sql&" and date between '"&sdate&"' and '"&ldate&"' " & searchSQL 
sql=sql&" union "
sql=sql&" select a.number from GiftToLog a with(nolock) inner join v_all_member_list b with(nolock) on a.to_user=b.user_id"
sql=sql&" where 1=1" 
...
admin_money = CDbl(Replace(GetString_List(sql),RowDel,""))    
...
<!--
    ...
    function goWrite() {
        ...
        if (document.getElementById("grade").options[document.getElementById("grade").selectedIndex].value == "") {
            alert("등급을 선택해 주세요!");
            document.getElementById("grade").focus();
            return;
        }
        if (document.getElementById("request_id").value == "" && document.getElementById("request_nick").value == "") {
            alert("ID를 입력해 주세요!");
        ...
        */
        ...
        if (Number(gold_amt) < Number(document.getElementById("gold_amt").value)) {
            if (document.getElementById("act_type").options[document.getElementById("act_type").selectedIndex].value == "del") {
                alert("회수머니가 보유머니를 초과하였습니다!");
                document.getElementById("gold_amt").focus();
                return;
            }
        }
        if (Number("<%=admin_money%>") < Number(document.getElementById("gold_amt").value)) {
            if (document.getElementById("act_type").options[document.getElementById("act_type").selectedIndex].value == "charge") {
    ...
    function chk_id() {
        ...
        //var url = "/$adm/patriot/view_usergold.asp?request_id=" + escape(id) + "&request_nick=" + nick + "&grade=" + document.getElementById("grade").options[document.getElementById("grade").selectedIndex].value;
        var url = "/$adm/patriot/view_usergold.asp?request_id=" + escape(id) + "&grade=" + document.getElementById("grade").options[document.getElementById("grade").selectedIndex].value;
        if (document.getElementById("grade").options[document.getElementById("grade").selectedIndex].value == "") {
            alert("등급을 선택해 주세요!");
            document.getElementById("grade").focus();
            return;
        ...
        */
        if (document.getElementById("act_type").options[document.getElementById("act_type").selectedIndex].value == "del") {
           // $.get("/$adm/patriot/view_usergold.asp?request_id=" + escape(id) + "&request_nick=" + escape(nick) + "&grade=" + document.getElementById("grade").options[document.getElementById("grade").selectedIndex].value, function (code, status) {
            $.get("/$adm/patriot/view_usergold.asp?request_id=" + escape(id) + "&grade=" + document.getElementById("grade").options[document.getElementById("grade").selectedIndex].value, function (code, status) {
                if (code != "" && code != null) {
                    if (code.substr(0, 2) != 0) {
                        ...
                        document.getElementById("possible_ex_amt").value = "0";
...
<form name="frm11" method="post" action="?" class="form-horizontal">
                    ...
                    <table id="boss_deal_pay_list_table" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                     ...
                     %>
                      ...
                      <%else 
                          ...
                          sql=sql&" select a.* from GiftToLog a with(nolock) inner join v_all_member_list b with(nolock) on a.from_user=b.user_id where 1=1"
                          sql=sql&" and date between '"&sdate&"' and '"&ldate&"'"&searchSQL
                          sql=sql&" union "
                          sql=sql&"   select a.* from GiftToLog a with(nolock) inner join v_all_member_list b with(nolock) on a.to_user=b.user_id where 1=1"
                          sql=sql&" and date between '"&sdate&"' and '"&ldate&"'"&searchSQL
                        ...
                        Record_String = RegList  
	                        ...
	                        For i = 0 to UBound(Rows)-1
                                ...
                                send_id     = Cols(1)
                                user_id     = Cols(2)
                                sql="select grade from v_all_member_list where user_id='"&send_id&"'"
                                send_grade = CINT(Replace(GetString_List(sql),RowDel,""))    
                                select case send_grade
                                        case 1
                                         send_grade_txt = "운영본사"
                                        case 15
                                         send_grade_txt = "본사"
                                ...
                                sql="select grade from v_all_member_list where user_id='"&user_id&"'"
                                take_grade = CINT(Replace(GetString_List(sql),RowDel,""))  
                                sql="select jisa_id from v_all_member_list where user_id='"&user_id&"'"
                                jisa_id_take = cstr(Replace(GetString_List(sql),RowDel,""))
                                sql="select jisa_id from v_all_member_list where user_id='"&send_id&"'"
                                jisa_id_send = cstr(Replace(GetString_List(sql),RowDel,""))
                                select case take_grade
                                        case 1
                                         take_grade_txt = "운영본사"
                                        case 15
                                         take_grade_txt = "본사"
                        ...
                        <tr bgcolor="#FCF7ED" height="25"> 
	                        ...
	                        <td align="center" style="color:<%if jisa_id_send="마스터" then%>blue<%else%>red<%end if%>">[<%=send_grade_txt%>]<%=send_id%></td>
                            <td align="center" style="color:<%if jisa_id_take="마스터" then%>blue<%else%>red<%end if%>">[<%=take_grade_txt%>]<%=user_id%></td>
                    ...
                    <form class="form-horizontal form-label-left" name="frm1" id="frm1" method="post" action="update_gold_del_charge.asp">     
                      ...
                      <div class="form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="first-name">회원 등급
                        </label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <select class="select2_single form-control" name="grade" id="grade">
                                <option value="">선택</option>
                                <!--option value="99">패트리어트</option>
                                <option value="0">루트본사</option>
                                <option value="1">본사</option-->
                                <option value="15">본사</option>
                      ...
                      <div class="form-inline form-group">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12 form-inline">
                            ...
                            <!--label class="form-control col-sm-2">닉네임 : </!--label><input type="text" name="request_nick" id="request_nick" class="form-control col-sm-2" /--> 
                                     <input type="button" id="btn_chk_id" class="btn btn-info form-control" onclick="chk_id()" value=" 보유 머니 조회  ">
                        </div>
                      </div>
                      <div class="ln_solid"></div>  
                      <div class="form-group">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <input type="text" name="possible_ex_amt" id="possible_ex_amt" class="form-control col-sm-2" size="20" maxlength="15" readonly="readonly" />&nbsp;머니
                        </div>
                      </div>
                      <div class="ln_solid"></div>
                      <div class="form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="first-name">충전/회수
                        </label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <select class="select2_single form-control" name="act_type" id="act_type">
                                <option value="charge">충전</option>
                                <option value="del">회수</option>
                            </select>
                        </div>
                      </div>
                      <div class="ln_solid"></div>
                      <div class="form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="first-name">충전/회수 머니
                        </label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                      ...
                      <div class="ln_solid"></div>
                      <div class="form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="first-name">충전/회수 사유
                        </label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
