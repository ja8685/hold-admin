...
if IDAuth = 1 then
    ...
    select case grade
            case 0
                sql="select deal_money from boss_distributor where user_id='"&request_id&"'"
                if act_type = "del" then
                upsql="update boss_distributor set deal_money = deal_money - "&gold_amt&" where user_id='"&request_id&"'"
                elseif act_type = "charge" then
                upsql="update boss_distributor set deal_money = deal_money + "&gold_amt&" where user_id='"&request_id&"'"
                end if
                rest_gold = Replace(GetString_List(sql),RowDel,"")
            case 1
                sql="select deal_money from distributor1 where user_id='"&request_id&"'"
                 if act_type = "del" then
                upsql="update distributor1 set deal_money = deal_money - "&gold_amt&" where user_id='"&request_id&"'"
                elseif act_type = "charge" then
                upsql="update distributor1 set deal_money = deal_money + "&gold_amt&" where user_id='"&request_id&"'"
                end if
                rest_gold = Replace(GetString_List(sql),RowDel,"")
            case 15
                sql="select deal_money from distributor15 where user_id='"&request_id&"'"
                 if act_type = "del" then
                upsql="update distributor15 set deal_money = deal_money - "&gold_amt&" where user_id='"&request_id&"'"
                elseif act_type = "charge" then
                upsql="update distributor15 set deal_money = deal_money + "&gold_amt&" where user_id='"&request_id&"'"
                end if            
                rest_gold = Replace(GetString_List(sql),RowDel,"")
             case 2
                sql="select deal_money from distributor2 where user_id='"&request_id&"'"
                 if act_type = "del" then
                upsql="update distributor2 set deal_money = deal_money - "&gold_amt&" where user_id='"&request_id&"'"
                elseif act_type = "charge" then
                upsql="update distributor2 set deal_money = deal_money + "&gold_amt&" where user_id='"&request_id&"'"
                end if   
                ...
                rest_gold = Replace(GetString_List(sql),RowDel,"")
            case 3
                sql="select deal_money from distributor3 where user_id='"&request_id&"'"
                if act_type = "del" then
                upsql="update distributor3 set deal_money = deal_money - "&gold_amt&" where user_id='"&request_id&"'"
                elseif act_type = "charge" then
                upsql="update distributor3 set deal_money = deal_money + "&gold_amt&" where user_id='"&request_id&"'"
                end if            
                rest_gold = Replace(GetString_List(sql),RowDel,"")
            case 4
                ...
                if act_type = "del" then
                    upsql="select isnull(user_money,0) from  users where user_id='"&request_id&"'"
                    user_money = cdbl(Replace(GetString_List(upsql),RowDel,""))
                    user_rest = user_money - cdbl(gold_amt)
                ...
                rest_gold = Replace(GetString_List(sql),RowDel,"")
            case 99
                sql="select deal_money from patriot_distributor"
                if act_type = "del" then
                upsql="update patriot_distributor set deal_money = deal_money - "&gold_amt
                elseif act_type = "charge" then
                upsql="update patriot_distributor set deal_money = deal_money + "&gold_amt
                end if                                      
    ...
    select case grade
            case 0
            sql="select deal_money from boss_distributor where name='"&request_nick&"'"
            if act_type = "del" then
            upsql="update boss_distributor set deal_money = deal_money - "&gold_amt&" where name='"&request_nick&"'"
            elseif act_type = "charge" then
            upsql="update boss_distributor set deal_money = deal_money + "&gold_amt&" where name='"&request_nick&"'"
            end if
            rest_gold = Replace(GetString_List(sql),RowDel,"")
            case 1
            sql="select deal_money from distributor1 where name='"&request_nick&"'"
             if act_type = "del" then
            upsql="update distributor1 set deal_money = deal_money - "&gold_amt&" where name='"&request_nick&"'"
            elseif act_type = "charge" then
            upsql="update distributor1 set deal_money = deal_money + "&gold_amt&" where name='"&request_nick&"'"
            end if
            rest_gold = Replace(GetString_List(sql),RowDel,"")
            case 15
            sql="select deal_money from distributor15 where name='"&request_nick&"'"
             if act_type = "del" then
            upsql="update distributor15 set deal_money = deal_money - "&gold_amt&" where name='"&request_nick&"'"
            elseif act_type = "charge" then
            upsql="update distributor15 set deal_money = deal_money + "&gold_amt&" where name='"&request_nick&"'"
            end if            
            rest_gold = Replace(GetString_List(sql),RowDel,"")
             case 2
            sql="select deal_money from distributor2 where name='"&request_nick&"'"
             if act_type = "del" then
            upsql="update distributor2 set deal_money = deal_money - "&gold_amt&" where name='"&request_nick&"'"
            elseif act_type = "charge" then
            upsql="update distributor2 set deal_money = deal_money + "&gold_amt&" where name='"&request_nick&"'"
            end if            
            rest_gold = Replace(GetString_List(sql),RowDel,"")
            case 3
            sql="select deal_money from distributor3 where name='"&request_nick&"'"
            if act_type = "del" then
            upsql="update distributor3 set deal_money = deal_money - "&gold_amt&" where name='"&request_nick&"'"
            elseif act_type = "charge" then
            upsql="update distributor3 set deal_money = deal_money + "&gold_amt&" where name='"&request_nick&"'"
            end if            
            rest_gold = Replace(GetString_List(sql),RowDel,"")
            ...
            if act_type = "del" then
                upsql="select isnull(user_money,0) from  users where user_nick='"&request_nick&"'"
                user_money = cdbl(Replace(GetString_List(upsql),RowDel,""))
                user_rest = user_money - cdbl(gold_amt)
    ...
    elseif act_type="charge" then
     ...
     if act_type="D" then
        upsql="update distributor1 set deal_money = deal_money - "&gold_amt&" where user_id='"&AdminID&"'"
        GameDBconn.execute(upsql)	
    elseif act_type="C" then
        upsql="update distributor1 set deal_money = deal_money - "&gold_amt&" where user_id='"&AdminID&"'"
        GameDBconn.execute(upsql)	
    end if      
    if request_nick <> "" then
        if grade=15 then
            upsql="select user_id from distributor15 where name='"&request_nick&"'"
        elseif grade=2 then
            upsql="select user_id from distributor2 where name='"&request_nick&"'"
        elseif grade=3 then
            upsql="select user_id from distributor3 where name='"&request_nick&"'"
        elseif grade=4 then
            upsql="select user_id from users where user_nick='"&request_nick&"'"
        end if
        request_id = Replace(GetString_List(upsql),RowDel,"")    
    end if
    sql = "insert into t_gold_del_charge_log (user_grade,send_grade,send_id,user_id,gold_amt,rest_gold,act_type,gold_desc,reg_date) "
    sql = sql&"values ("&grade&","&IDAuth&",'"&adminID&"','"&request_id&"',"&gold_amt&","&rest_gold&",'"&act_type&"','"&del_charge_desc&"',getdate())"
    ...
    sql="select IDENT_CURRENT('t_gold_del_charge_log')"
    ...
    if grade = 4 then
        ...
        set rs12 = nothing
        if act_type="D" then
            sql21="insert into tb_outcome_log (parent_no,take_id,outcome_amt,status,bonsa_id,jisa_id,chongpan_id,shop_id,bonsa_rate,chongpan_rate,shop_rate,bonsa_commission,chongpan_commission,shop_commission,approval_date)"
            sql21=sql21&" values ("&no&",'"&request_id&"',"&gold_amt&",'D','"&AdminID&"','"&jisa_id&"','"&chongpan_id&"','"&shop_id&"',0,0,0,0,0,0,getdate())"
        elseif act_type="C" then
