...
connectionGameDB("PNC")
  ...
  GameDBconn.BeginTrans
        sql = "update tb_notice set title='"&title&"',  contents='"&contents&"',  status='"&state&"' where idx="&no
    ...
    If GameDBconn.Errors.Count <> 0 Then         
        ...
        response.Write "<script>alert(""공지사항 수정이 완료되었습니다."");parent.document.location.href='notice_list.asp'</script>"
    end if
    closeGameDB        
%>