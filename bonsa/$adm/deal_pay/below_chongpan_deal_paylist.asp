...
if lyear="" then lyear=Request.form("lyear")
...
parameters = "f_storecode_key=" & f_storecode_key & "&search_date=" & search_date & "&keyfield=" & keyfield & "&keyword=" & keyword & "&period=" & period & "&page_size=" & page_size & "&u_id=" & u_id& "&adminno="&adminno&"&adminID="&adminID
...
elseif IDAuth = 2 then
    ...
    sql = sql&" union all "
    sql = sql&" select chongpan_id,shop_id,0 as income_amt_save,0 as incoume_amt,0 as outcome_amt,sum(money) as outcome_save_amt,0 as rest_amt,0 as betting_amt,0 as deal_use_money"
    sql = sql&" ,0 as bonsa_commission,0 as chongpan_commision,0 as shop_commission,'Y' as status,date "
    sql = sql&" from GiftToLog a inner join v_member_list01 b on a.from_user=b.user_id "
    sql = sql&" where 1=1 "
    sql = sql&" group by chongpan_id,shop_id,a.date having chongpan_id='"&adminID&"' and shop_id<>'' and a.date between '"&sdate&"' and '"&ldate&"' "
    sql = sql&" union all "
    sql = sql&" select chongpan_id,shop_id,sum(money) as income_amt_save,0 as incoume_amt,0 as outcome_amt,0 as outcome_save_amt,0 as rest_amt,0 as betting_amt,0 as deal_use_money"
    sql = sql&" ,0 as bonsa_commission,0 as chongpan_commision,0 as shop_commission,'Y' as status,date "
        ...
        <div class="row">
            <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  ...
                  <div class="x_content">                        
                    <form name="frm1" method="post" action="<%=move_url%>">
                       ...
                       <%
                         ...
                         %>
                            <tr bgcolor="#FCF7ED" height="25"> 
	                            <td align="center" height="25" colspan="11"><b>등록된 정산내역이 없습니다.</b></td>
                            </tr>
                          <%else 
	                            ...
	                            For i = 0 to UBound(Rows)-1
                                    ...
                                    if IDAuth = 1 then
                                        ...
                                        sql="     select c.user_id as bonsa_id,b.user_id as chongpan_id,a.user_id as shop_id,'dd' as ddd,c.distribute1_rate,b.distribute2_rate ,a.distribute3_rate,a.deal_money from distributor3 a inner join distributor2 b on a.distribute2_link=b.number"
                                    ...
                                    elseif IDAuth = 2 then
                                         ...
                                         income_amt=0
                                         income_amt2=0
                                         sql="     select c.user_id as bonsa_id,b.user_id as chongpan_id,a.user_id as shop_id,'dd' as ddd,c.distribute1_rate,b.distribute2_rate ,a.distribute3_rate,d.user_rate,a.deal_money from distributor3 a inner join distributor2 b on a.distribute2_link=b.number"
                                        ...
                                        if not rs2.eof then
                                            ...
                                            shop_money = cdbl(trim(rs2("deal_money")))
                                        ...
                                        set rs2 = nothing
                                    sql="select isnull(sum(user_money+user_save),0) from users a with(nolock) inner join distributor3 b on a.user_distributor3_link=b.number where b.user_id='"&Cols(1)&"'"            
                          ...
                          %>
                            <tr bgcolor="#FCF7ED" height="25"> 
                                ...
                                <td align="center"><%=user_id&"["&distribute3_rate&"%]"%></td>
