...
sday = Request("sday")
stime1 = Request("stime")
lyear = Request("lyear")
lmonth = Request("lmonth")
lday = Request("lday")
ltime = Request("ltime")
if stime1 = "" then stime1 = "13"
...
sdate = syear & "-" & putZero(smonth) & "-" & putZero(sday) & " " & putZero(stime1) & ":00:00"
else
...
sdate = syear1 & "-" & putZero(smon1) & "-" & putZero(sday1) & " " & putZero(stime1) & ":00:00"
end if
...
parameters = parameters & "&lyear=" & lyear & "&lmonth=" & lmonth & "&lday=" & lday & "&page_size=" & page_size & "&u_id=" & u_id & "&stime=" & stime & "&ltime=" & ltime 
parameters = parameters & "&realuser=" & realuser &"&take_id=" & server.URLEncode(take_id)

move_url = "bonsa_chargelist.asp?" & parameters
sql=" select count(A.idx) from TB_Deposit A, v_member_list01 B where A.send_id = B.user_id "
authtitle = "본사"
...
if take_id <> "" then 
	searchSQL = searchSQL & " and A.send_id like '%"& take_id &"%'"
end If
...
<!--
    ...
    function charge_put(str, no) {
        var url = "bonsa_charge_ok.asp?gubun=" + str + "&no=" + no;
        if (str == "Y") {
            if (confirm("충전신청을 승인하시겠습니까?")) {
                document.location.href = url;
            }
        }
        if (str == "C") {
            if (confirm("충전신청을 취소하시겠습니까?")) {
                document.location.href = url;
            }
        }
        if (str == "D") {
            if (confirm("충전신청을 삭제하시겠습니까?")) {
                document.location.href = url;
            }
        }
                    ...
                    <div class="row">
                      ...
                      <thead>
                        <tr class="alert alert-info alert-dismissible fade in">
	                        ...
	                        <th style="text-align:center" height="25"><b>입금자명</b></th>
                            ...
                            <th style="text-align:center" height="25"><b>승인일자</b></th>                        
                        </tr>
                      </thead>
                      <tbody>
                <%
                 ...
                 %>
                  ...
                  <%else 
                  if take_id <> "" Then
	                sql_where = " and A.send_id like '%"&take_id&"%' and A.request_date between '"& sdate &"' and '"& ldate &"' "
                    ...
                    'sql="select top "&PageSize&" 's' as sdfsd, 'sdfsd' as sdfdd, A.* from TB_Deposit A with(nolock), v_member_list01 B where A.send_id = B.user_id and A.idx not in (select top "&int((page-1)*PageSize)&" A.idx from TB_Deposit A with(nolock), v_member_list01 B where A.send_id = B.user_id " & sql_where & " order by A.request_date desc) " & sql_where & " order by A.request_date desc "
                  else
                    ...
                    'sql="select top "&PageSize&" 's' as sdfsd, 'sdfsd' as sdfdd, A.* from TB_Deposit A with(nolock), v_member_list01 B where A.send_id = B.user_id and A.idx not in (select top "&int((page-1)*PageSize)&" A.idx from TB_Deposit with(nolock), v_member_list01 B where A.send_id = B.user_id " & sql_where & " order by request_date desc) " & sql_where & " order by A.request_date desc "
                  end If

                    sql="select * from (select row_number() over(order by A.request_date desc) as num,'s' as sdfsd, 'sdfsd' as sdfdd, A.* from TB_Deposit A, v_member_list01 B where A.send_id = B.user_id " & sql_where & ") t WHERE	T.num BETWEEN("&int((page-1)*PageSize)&"+1) AND ("&page&" * "&PageSize&") ORDER BY	T.request_date desc"
                    ...
                    Record_String = RegList  
	                    ...
	                    For i = 0 to UBound(Rows)-1
                            ...
                            user_nick= replace(GetString_List(sql),RowDel,"")
                             sql=" select user_phone from users where user_id='"&take_id&"'"
                            user_phone= replace(GetString_List(sql),RowDel,"")
                            end if
                            accountname= Cols(5)
                            exchange_amt= Cols(6)
                            state= Cols(7)
                            ...
                            request_date= Cols(17)
                            approval_date= Cols(18)    
                        sql="select c.num from users a inner join TB_Member_Accountno b on a.number=b.midx inner join (select ROW_NUMBER() OVER( ORDER BY idx) as num,* from TB_AccountInfo) c on b.accountno=c.num where user_id='"&take_id&"'"        
                        user_accno= replace(GetString_List(sql),RowDel,"")
                  %>
                    <tr height="25" style="font-weight: bold"> 
                        ...
                        <td align="center"><%=accountname%><font color="blue">(<%=user_accno%>)</font></td>
                        <td align="center"><%=user_phone%></td>
                        <td align="right" style="padding-right:20px"><%=formatnumber(exchange_amt,0)%></td>
                        <td align="center"><%=state%></td>
                        <td align="center"><%if state <>"완료" and state<>"취소" then%>
                                                <button onclick="charge_put('Y',<%=no%>);" class="btn btn-warning form-control btn-sm">승인</button>
                                            <%end if%>
                                            <%if state <> "완료" and state<>"취소" then%>
                                                <button onclick="charge_put('C',<%=no%>);" class="btn btn-warning form-control btn-sm">취소</button>
                                            <%end if %>
                                            <%if state <>"완료" then%>
                                                <button onclick="charge_put('D',<%=no%>);" class="btn btn-danger form-control btn-sm">삭제</button>
                                            <%end if %></td>
                        ...
                        <td align="center"><%=approval_date%></td>
                         ...
                         if state = "완료" then
                         tot_charge_amt = tot_charge_amt + cdbl(exchange_amt)
                         end if
	                    Next
                   ...
                   %>
                    <tr height="25"> 
                        ...
                        <td align="center">&nbsp;</td>
                        <td align="center">&nbsp;</td>                        
                        <td align="right" style="padding-right:20px;"><font color="red"><%=formatnumber(tot_charge_amt,0)%></font></td>        
                        <td align="center">&nbsp;</td>
                        <td align="center">&nbsp;</td>
                        <td align="center">&nbsp;</td>
                        <td align="center">&nbsp;</td>
