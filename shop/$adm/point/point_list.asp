...
sql=" select count(a.number) from PointChangeLog a with(nolock) inner join v_member_list01 b with(nolock) on a.user_id=b.user_id inner join users c"
sql=sql&" with(nolock) on a.user_id=c.user_id where a.date between '"&sdate&"' and '"&ldate&"' and c.user_distributor3_link="&request.Cookies(admincheck)("adminNo")
if keyword <> "" then 
                    ...
                    <table id="boss_deal_pay_list_table" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                     ...
                     %>
                      ...
                      <%else 
                         ...
                         sql=sql&" with(nolock) on a.user_id=b.user_id inner join users c with(nolock) on a.user_id=c.user_id where a.date between '"&sdate&"' "
                         sql=sql&" and '"&ldate&"' and c.user_distributor3_link="&request.Cookies(admincheck)("adminNo")&"  "&searchSQL&" and a.number not in "
                         sql=sql&" (select top "&int((page-1)*PageSize)&" a.number from PointChangeLog a with(nolock) inner join v_member_list01 b with(nolock) on"
                         sql=sql&"  a.user_id=b.user_id inner join users c on a.user_id=c.user_id where  "
                         sql=sql&"  c.user_distributor3_link="&request.Cookies(admincheck)("adminNo")&" and a.date between '"&sdate&"' and '"&ldate&"' "&searchSQL&" order by a.date desc) order by date desc"
