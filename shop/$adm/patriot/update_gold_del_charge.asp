...
upsql="select deal_money from distributor3 where user_id='"&AdminID&"'"
admin_rest_gold = trim(Replace(GetString_List(upsql),RowDel,""))
    ...
    elseIf ret_val=1 Then                 
        ...
        response.Write "<script>alert(""골드 선물이 완료되었습니다."");parent.document.location.reload();top.menu.document.getElementById('admin_rest_money').innerHTML='"&formatnumber(admin_rest_gold,0)&"';</script>"
    end if
    closeGameDB
