...
chongpan_id = request.Cookies(admincheck)("adminID")
u_id = request.Cookies(admincheck)("adminNO")
sql = "select distribute1_link from distributor2 where number="&request.Cookies(admincheck)("adminNO")
bonsa_no = CDbl(Replace(GetString_List(sql),RowDel,""))
sql = "select user_id from distributor1 where number="&bonsa_no 
bonsa_id = CSTR(Replace(GetString_List(sql),RowDel,""))
end if
                ...
                <div class="x_panel">
                  ...
                  <div class="x_content">                        
                        <div class="row pull-right">
                            <form name="frm1" method="post" action="?u_id=<%=server.urlencode(u_id)%>&chongpan_id=<%=server.urlencode(chongpan_id)%>&bonsa_id=<%=server.urlencode(bonsa_id)%>" class="form-horizontal">
	                                <input type="hidden" name="user_id" />                       
                                    <div class="form-inline form-group" >  
                                        <input type=button name=headregist value="매장 추가" onClick="document.location.href = 'insert_shop.asp?bonsa_no=<%=bonsa_no%>&bonsa_id=<%=server.urlencode(bonsa_id)%>&chongpan_no=<%=server.urlencode(u_id)%>'" class="btn btn-success col-sm-2" />
                                ...
                                &nbsp;&nbsp;<input type="text" name="keyword" size="20" value="<%=keyword%>" onFocus="this.select()" class="form-control col-sm-2" />
                    ...
                    <%
                        ...
                        sql=sql&" order by a.register_date desc"
                         ...
                         %>
                            <tr height="25"> 
                                <td id="charge_td2" align="center" height="25"><b>등록된 매장이 없습니다.</b></td>
                            </tr>
                          <%else 
                            ...
                            <tr height="25"> 
                                ...
                                <td align="center"><%=cnt_member%></td>
  	                            <td style="text-align:center">                                      
                                      <input type=button name=headregist value=" 보기 " onClick="document.location.href ='below_memberlist.asp?shop_no=<%=no%>&u_id=<%=server.urlencode(user_id)%>&bonsa_id=<%=server.urlencode(bonsa_id)%>&chongpan_id=<%=server.urlencode(chongpan_id)%>'" class="btn btn-primary btn-xs" />
  	                            </td>				 
