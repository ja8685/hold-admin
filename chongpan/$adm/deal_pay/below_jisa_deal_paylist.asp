...
elseif IDAuth = 2 then
    ...
    sql = sql&" union all "
    sql = sql&" select '"&adminID&"' as chongpan_id,c.user_id as shop_id,sum(gold_amt) as income_amt_save,0 as incoume_amt,0 as outcome_amt,0 as outcome_save_amt,sum(user_money)+sum(user_save) as rest_amt,0 as betting_amt,0 as deal_use_money"
    ...
    sql = sql&" union all "
    sql = sql&" select '"&adminID&"' as chongpan_id,c.user_id as shop_id,0 as income_amt_save,0 as incoume_amt,0 as outcome_amt,sum(gold_amt) as outcome_save_amt,sum(user_money)+sum(user_save) as rest_amt,0 as betting_amt,0 as deal_use_money"
    sql = sql&" ,0 as bonsa_commission,0 as chongpan_commision,0 as shop_commission,'Y' as status,'' as reg_date "
