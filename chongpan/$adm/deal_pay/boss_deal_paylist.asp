...
elseif IDAuth = 2 then
    ...
    sql = sql&" union all "
    sql = sql&" select '"&adminID&"' as chongpan_id,c.user_id as shop_id,sum(a.money) as income_amt_save,0 as incoume_amtt,0 as outcome_amt,0 as outcome_save_amt,sum(user_money) as rest_amt,0 as betting_amt,0 as deal_use_money"
    ...
    sql = sql&" where b.user_distributor2_link="&adminNum
    sql = sql&" and a.date between '"&sdate&"' and '"&ldate&"' "
    'sql = sql&searchshop
    sql = sql&" group by c.user_id "   
    sql = sql&" union all "
    sql = sql&" select '"&adminID&"' as chongpan_id,c.user_id as shop_id,0 as income_amt_save,0 as incoume_amtt,0 as outcome_amt,sum(a.money) as outcome_save_amt,sum(user_money) as rest_amt,0 as betting_amt,0 as deal_use_money"
    sql = sql&" ,0 as bonsa_commission,0 as chongpan_commision,0 as shop_commission,'Y' as status,'' as reg_date "
                  ...
                  <div class="x_content">                        
                    ...
                    <table id="boss_deal_pay_list_table" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                      ...
                      <%
                         ...
                         %>
                            <tr height="25"> 
	                            <td id="charge_td2" align="center" height="25"><b>등록된 정산내역이 없습니다.</b></td>
                            </tr>
                          <%else 
	                            ...
	                            For i = 0 to UBound(Rows)-1
                                    ...
                                    elseif IDAuth = 2 then
                                        ...
                                        if not rs2.eof then
                                            ...
                                            distribute2_amt=fix(cdbl(betting_amt)*distribute2_rate/100)
                                            ...
                                            chongpan_id=trim(rs2("chongpan_id"))
                                            shop_id=trim(rs2("shop_id"))
                                            sql="select isnull(sum(deal_money),0) from distributor2 with(nolock) where user_id='"&chongpan_id&"'"            
                                    
                                            set rs3 = GameDBconn.execute(sql)
                                            if not rs3.eof then
                                                chongpan_money = cdbl(trim(rs3(0)))
                                            else
                                                chongpan_money= 0
                                            end if
                                            set rs3 = nothing                                     
                                            sql="select isnull(sum(deal_money),0) from distributor3 with(nolock) where user_id='"&shop_id&"'"            
                              ...
                              income_amt = shop_money                                                         
                               distribute2_amt=  chongpan_money
                          %>
                            <tr height="25"> 
                                ...
                                <td align="right" style="padding-right:20px"><%=FormatNumber(tot_betting_amt,0)%></td>      
                                <td align="right" style="padding-right:20px"><%=FormatNumber(deal_use_amt,0)%></td>      
                                <td align="right"><%=FormatNumber(income_amt,0)%><!--font color="blue">(<%=distribute3_rate%>%)</font--></td>
                                <td align="right" style="padding-right:20px"><%=FormatNumber(distribute4_amt,0)%></td>
                                 ...
                                 sum_income_amt=cdbl(sum_income_amt+distribute2_amt)         
                                 if IDAuth = 1 then
                                 sum_income_amt2=cdbl(sum_income_amt2+income_amt)         
                                 ...
                                 tot_betting_amt=0
                                 tot_deal_use_amt = tot_deal_use_amt+deal_use_amt
                                 deal_use_amt=0
	                            Next
                           ...
                           %>
                            <tr bgcolor="#FCF7ED" height="25"> 
                                ...
                                <td align="right"><b><%=FormatNumber(sum_income_amt2,0)%></b></td>        
