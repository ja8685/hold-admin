...
PageSize = page_size
    ...
    sql=sql&" group by jisa_id,chongpan_id,shop_id,take_id,user_account_name,status having jisa_id IN (" & request.Cookies(admincheck)("BubonJisaId") & ")"
    ...
    sql=sql&" group by a.jisa_id,a.chongpan_id,a.shop_id,a.take_id,user_account_name,a.status having a.jisa_id IN (" & request.Cookies(admincheck)("BubonJisaId") & ")"
    ...
    sql=sql&" group by b.jisa_id,b.chongpan_id,b.shop_id,a.user_id ,user_account_name,a.status having b.jisa_id IN (" & request.Cookies(admincheck)("BubonJisaId") & ") and a.status=1 and b.chongpan_id<>''"
    ...
    sql = sql&" having a.jisa_id IN (" & request.Cookies(admincheck)("BubonJisaId") & ") and a.chongpan_id<>''"
    ...
    sql = sql&" GiftToLog a inner join users b on a.to_user=b.user_id inner join v_member_list01 c on b.user_id=c.user_id where a.money>0 and user_distributor1_link="&request.Cookies(admincheck)("adminNO")&" and a.date between"
    ...
    sql = sql&" group by jisa_id,chongpan_id,shop_id,a.to_user,user_account_name"
     sql = sql&" union all "
    sql = sql&" select jisa_id,chongpan_id,shop_id,a.to_user as take_id,user_account_name,0 as income_amt_save,0 as income_amt,0 as outcome_amt,-sum(a.money) as outcome_amt_save,'Y' as status,0 as betting_amt,0 as tot_deal_money  from "
    sql = sql&" GiftToLog a inner join users b on a.to_user=b.user_id inner join v_member_list01 c on b.user_id=c.user_id where a.money<0 and user_distributor1_link="&request.Cookies(admincheck)("adminNO")&" and a.date between"
    sql = sql&" '"&sdate&"' and '"&ldate&"'"
    if keyword <> "" then 
    sql=sql&" and "&keyfield&" = '"&keyword&"'"
    ...
    sql = sql&" GiftToLog a inner join users b on a.from_user=b.user_id inner join v_member_list01 c on b.user_id=c.user_id where a.money<0 and user_distributor1_link="&request.Cookies(admincheck)("adminNO")&" and a.date between"
    sql = sql&" '"&sdate&"' and '"&ldate&"'"
    if keyword <> "" then 
    sql=sql&" and "&keyfield&" = '"&keyword&"'"
    end if
    sql = sql&" group by jisa_id,chongpan_id,shop_id,a.from_user,user_account_name"
     sql = sql&" union all "
    sql = sql&" select jisa_id,chongpan_id,shop_id,a.from_user as take_id,user_account_name,sum(a.money) as income_amt_save,0 as income_amt,0 as outcome_amt,0 as outcome_amt_save,'Y' as status,0 as betting_amt,0 as tot_deal_money  from "
    sql = sql&" GiftToLog a inner join users b on a.from_user=b.user_id inner join v_member_list01 c on b.user_id=c.user_id where a.money>0 and user_distributor1_link="&request.Cookies(admincheck)("adminNO")&" and a.date between"
    sql = sql&" '"&sdate&"' and '"&ldate&"'"
    if keyword <> "" then 
    sql=sql&" and "&keyfield&" = '"&keyword&"'"
                    ...
                    <table id="boss_deal_pay_list_table" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                      <thead>
                        <tr class="alert alert-info alert-dismissible fade in">
                            ...
                            <!--th style="text-align:center" height="25"><b>일자</b></th-->
                        ...
                        <%
                         ...
                         %>
	                        ...
	                        For i = 0 to UBound(Rows)-1
                                ...
                                exchange_amt = cdbl(Cols(7))
                                exchange_amt_save = cdbl(Cols(8))
                                ...
                                if tot_user_inout<0 then
                                    ...
                                    sql=" select isnull(user_money,0) from users where user_id='"&user_id&"'"
                      ...
                      %>
                       ...
                       %>
                        <tr bgcolor="#FCF7ED" height="25"> 
                            ...
                            <td align="right" style="padding-right:20px;color:red"><b><%=FormatNumber(sum_tot_amt,0)%></b></td>             
