...
elseif IDAuth = 2 then
    ...
    sql = sql&" union all "
    sql = sql&" select chongpan_id,shop_id,sum(gold_amt) as income_amt_save,0 as incoume_amt,0 as outcome_amt,0 as outcome_save_amt,0 as rest_amt,0 as betting_amt,0 as deal_use_money"
    sql = sql&" ,0 as bonsa_commission,0 as chongpan_commision,0 as shop_commission,'Y' as status,reg_date "
    sql = sql&" from t_gold_del_charge_log a inner join v_member_list01 b on a.user_id=b.user_id "
    sql = sql&" where act_type='C'"
    sql = sql&" group by chongpan_id,shop_id,a.reg_date having chongpan_id='"&adminID&"' and shop_id<>'' and a.reg_date between '"&sdate&"' and '"&ldate&"' "
    sql = sql&" union all "
    sql = sql&" select chongpan_id,shop_id,0 as income_amt_save,0 as incoume_amt,0 as outcome_amt,sum(gold_amt) as outcome_save_amt,0 as rest_amt,0 as betting_amt,0 as deal_use_money"
    sql = sql&" ,0 as bonsa_commission,0 as chongpan_commision,0 as shop_commission,'Y' as status, reg_date "
    sql = sql&" from t_gold_del_charge_log a inner join v_member_list01 b on a.send_id=b.user_id "
    sql = sql&" where act_type='C'"
    sql = sql&" group by chongpan_id,shop_id,a.reg_date having chongpan_id='"&adminID&"' and shop_id<>'' and a.reg_date between '"&sdate&"' and '"&ldate&"' "
    'sql = sql&searchshop
    'sql = sql&" union all "
	                            ...
	                            For i = 0 to UBound(Rows)-1
                                    ...
                                    elseif IDAuth = 2 then
                                        ...
                                        if not rs2.eof then
                                            ...
                                            distribute3_amt=fix(cdbl(betting_amt+tot_deal_money)*distribute3_rate/100)                    
