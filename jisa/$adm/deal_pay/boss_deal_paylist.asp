...
IDAuth = 15
	...
	sql = sql&" from ( "
    sql = sql&" select chongpan_id,shop_id,0 as income_amt_save,sum(outcome_amt) as income_amt,0 as outcome_amt,0 as outcome_save_amt,0 as rest_amt,0 as betting_amt,0 as deal_use_money,sum(bonsa_commission) as bonsa_commission,sum(chongpan_commission) as chongpan_commission,sum(shop_commission) as shop_commission,approval_date,status from tb_deposit_log with(nolock) where 1=1"
    'sql = sql&searchshop
    sql = sql&" group by jisa_id,chongpan_id,shop_id,approval_date,status having jisa_id IN ('" & request.Cookies(admincheck)("adminID") & "') and status='Y' and shop_id<>'' and approval_date between '"&sdate&"' and '"&ldate&"' "
    sql = sql&" union all "
    sql = sql&" select chongpan_id,shop_id,0 as income_amt_save,0 as income_amt,sum(outcome_amt) as outcome_amt,0 as outcome_save_amt,0 as rest_amt,0 as betting_amt,0 as deal_use_money,0 as bonsa_commission,0 as chongpan_commission,0 as shop_commission,approval_date,status from tb_outcome_log with(nolock) where 1=1"
    'sql = sql&searchshop
    sql = sql&" group by jisa_id,chongpan_id,shop_id,approval_date,status having jisa_id IN ('" & request.Cookies(admincheck)("adminID") & "')  and status='Y' and shop_id<>'' and approval_date between '"&sdate&"' and '"&ldate&"' "
    sql = sql&" union all "
    ...
    sql = sql&" chongpan_commission,0 as shop_commission,reg_date,'Y' as status from deal_money_ins_log with(nolock) where 1=1 "
    'sql = sql&searchshop
    sql = sql&" group by jisa_id,chongpan_id,shop_id,reg_date "
    sql = sql&" having jisa_id IN ('" & request.Cookies(admincheck)("adminID") & "')  and reg_date between '"&sdate&"' and '"&ldate&"'"
    sql = sql&" union all "
    ...
    sql = sql&" from GiftToLog a with(nolock) inner join users b with(nolock) on a.to_user=b.user_id inner join distributor3 c with(nolock) on b.user_distributor3_link=c.number inner join distributor2 d with(nolock) on b.user_distributor2_link=d.number"
    sql = sql&" where b.user_distributor15_link IN (" & request.Cookies(admincheck)("adminNo") & ")"
    ...
    sql = sql&" ,0 as bonsa_commission,0 as chongpan_commision,0 as shop_commission,'Y' as status,'' as reg_date "
    sql = sql&" from GiftToLog a with(nolock) inner join users b with(nolock) on a.from_user=b.user_id inner join distributor3 c with(nolock) on b.user_distributor3_link=c.number inner join distributor2 d with(nolock) on b.user_distributor2_link=d.number"
    sql = sql&" where b.user_distributor15_link IN (" & request.Cookies(admincheck)("adminNo") & ")"
    sql = sql&" and a.date between '"&sdate&"' and '"&ldate&"' "
    'sql = sql&searchshop
    sql = sql&" group by c.user_id,d.user_id "    
                            ...
                            <div class="form-inline form-group">
                                ...
                                &nbsp;시                                     
                                 ...
                                 &nbsp;&nbsp;총판:	
                                  ...
                                  <%
                                  sql="select distinct a.user_id from distributor2 a left outer join distributor15 b on a.distribute15_link=b.number where 1=1 and b.user_id is not null and b.user_id IN ('" & request.Cookies(admincheck)("adminID") & "')"                          
                                  'if s_chongpan_id <> "" then
                                ...
                                <select class="select2_single form-control"  name="s_shop_id" onchange="document.frm1.submit()">
                                   <option value="">=선택=</option>
                                  <%
                                  sql="select distinct c.user_id from distributor2 a , distributor15 b , distributor3 c where 1=1 and a.distribute15_link=b.number and a.number = c.distribute2_link and b.user_id IN ('" & request.Cookies(admincheck)("adminID") & "')"
                                 ' if s_chongpan_id <> "" then
                                 '   sql = sql&" and a.user_id='"&s_chongpan_id&"'"
                                 ' end if
                            ...
                            <tr bgcolor="#FCF7ED" height="25"> 
                               ...
                               <%else 
	                                ...
	                                For isdsd = 0 to UBound(Rows)-1
                                        ...
                                        if IDAuth = 15 then
                                            ...
                                            sql="select isnull(sum(user_money),0) from users a with(nolock) inner join distributor2 b with(nolock) on a.user_distributor2_link=b.number where b.user_id='"&Cols(0)&"'"            
                                            ...
                                            sql="select number from distributor2 with(nolock) where user_id='"&Cols(0)&"'"            
                                            set rs3 = GameDBconn.execute(sql)
                                            if not rs3.eof then
                                                chongpan_no = trim(rs3(0))
                                            else
                                                chongpan_no= 0
                                            end if
                                            set rs3 = nothing
                                            ...
                                            sql=sql&" ,(select SUM(a.deal_money) from distributor2 a with(nolock) inner join distributor15 b with(nolock) on a.distribute15_link=b.number where a.user_id='"&Cols(0)&"') as dis2_sum"&vbCrLf
                                            sql=sql&",(select SUM(a.deal_money) from distributor3 a with(nolock) inner join distributor2 b with(nolock) on a.distribute2_link=b.number where b.user_id='"&Cols(0)&"') as dis3_sum,"&vbCrLf
                                            sql=sql&"  d.number as jisano from distributor3 a with(nolock) inner join distributor2 b with(nolock) on a.distribute2_link=b.number"&vbCrLf
                                            sql=sql&"  inner join distributor15 d with(nolock) on a.distribute15_link=d.number inner join distributor1 c with(nolock) on a.distribute1_link=c.number where b.user_id='"&Cols(0)&"'"&vbCrLf            
                              ...
                              %>
                                <tr height="25" style="font-weight: bold"> 
                                    ...
                                    <td align="right" style="padding-right:20px"><input type="button" value="하위보기" class="btn btn-info form-control"  
                                        onclick="document.location.href = 'below_chongpan_deal_paylist.asp?keyfield=<%=keyfield%>&keyword=<%=keyword%>&adminno=<%=chongpan_no%>&adminid=<%=bonsa_id%>&syear=<%=syear%>&smonth=<%=smonth%>&sday=<%=sday%>&stime=<%=stime%>&lyear=<%=lyear%>&lmonth=<%=lmonth%>&lday=<%=lday%>&ltime=<%=ltime%>&page_size=<%=page_size%>&page=<%=page%>'" /></td>                
                                </tr>
                                     <%         
                               ...
                               %>   
                                <tr bgcolor="#FCF7ED" height="25"> 
                                    ...
                                    <td align="right" style="padding-right:20px;color:red"><b><%=FormatNumber(real_sum_charge_amt_save,0)%></b></td>
                                    ...
                                    <td align="right" style="padding-right:20px;color:red"><b><%=FormatNumber(real_sum_income_amt2,0)%></b></td>                
